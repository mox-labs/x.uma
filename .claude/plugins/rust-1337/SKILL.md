...
## x.uma Project Learnings

Patterns discovered while building the r.umi matcher engine.

### Type Erasure via Enum (Envoy Pattern)

When you need heterogeneous collections but want to avoid `dyn Trait` complexity:

```rust
// Type erasure at data level — simpler than Tower/Bevy patterns
...
// DataInput is generic (domain-specific)
pub trait DataInput<Ctx>: Send + Sync {
    fn get(&self, ctx: &Ctx) -> MatchingData;
}

// InputMatcher is NON-GENERIC (domain-agnostic, shareable!)
pub trait InputMatcher: Send + Sync {
    fn matches(&self, value: &MatchingData) -> bool;
}
```

**Why it works**: Same `ExactMatcher` works for HTTP, Claude, test contexts. No GATs needed.

### Don't Write `unsafe impl Send/Sync`

Almost always wrong. The compiler derives Send/Sync automatically when all fields are Send/Sync.

```rust
// WRONG — adds unnecessary restrictive bounds
unsafe impl<Ctx: Send> Send for MyType<Ctx> {}

// RIGHT — let compiler derive (no code needed)
// MyType is Send when its fields are Send
```

**When unsafe impl IS needed**: Raw pointers, FFI types, interior mutability tricks. If your "SAFETY" comment just says "all fields are Send/Sync" — delete the unsafe impl.

### no_std + alloc Feature Flags
...
default = ["std"]
std = []
alloc = []  # For no_std environments with allocator

// In code — WRONG
#[cfg(not(feature = "std"))]
use alloc::vec::Vec;  // Assumes alloc always available!
...
Code blocks in `///` comments are compiled and run as tests:
...
- Use `ignore` for examples that can't run standalone
- Use `no_run` for examples that compile but shouldn't execute
- Broken doc examples fail CI — keeps docs accurate

### Making Illegal States Unrepresentable (xDS Example)

When a proto uses `oneof`, model it as a Rust enum:

```protobuf
// Proto: exclusive choice
message OnMatch {
  oneof on_match {
    Matcher matcher = 1;
    Action action = 2;
...
```rust
// Rust: enum enforces exclusivity at compile time
pub enum OnMatch<Ctx, A> {
    Action(A),
    Matcher(Box<Matcher<Ctx, A>>),
}
// Can't have both action AND matcher — type system prevents it
```