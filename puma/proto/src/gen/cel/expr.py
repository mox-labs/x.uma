...
# sources: cel/expr/syntax.proto, cel/expr/checked.proto
...
class Expr(betterproto.Message):
    ...
    within expressions must resolve to a [Decl][cel.expr.Decl] provided at
    type-check for an expression to be valid. A reference may either be a bare
    identifier `name` or a qualified identifier `google.api.name`. References
    may either refer to a value or a function declaration. For example, the
    expression `google.api.name.startsWith('expr')` references the declaration
    `google.api.name` within a [Expr.Select][cel.expr.Expr.Select] expression,
    and the function declaration `startsWith`.
    """
...
class ExprComprehension(betterproto.Message):
    ...
    # The name of the first iteration variable. For the single iteration variable
    # macros, when iter_range is a list, this variable is the list element and
    # when the iter_range is a map, this variable is the map key.
    iter_var: str = betterproto.string_field(1)
    # The name of the second iteration variable, empty if not set. This field is
...
class Constant(betterproto.Message):
    ...
    as these aggregate types may contain [Expr][cel.expr.Expr] elements which
    require evaluation and are thus not constant. Examples of constants
    include: `"hello"`, `b'bytes'`, `1u`, `4.2`, `-2`, `true`, `null`.
    """

    # null value.
    null_value: protobuf.NullValue = betterproto.enum_field(
        1, group="constant_kind", wraps=betterproto.TYPE_NULL
    )
    # boolean value.
...
class SourceInfo(betterproto.Message):
    ...
    # The column may be derived from `id_positions[id] - line_offsets[i]`.
...
class Type(betterproto.Message):
    ...
    # Dynamic type.
    dyn: protobuf.Empty = betterproto.message_field(1, group="type_kind")
    # Null value.
    null: protobuf.NullValue = betterproto.enum_field(
        2, group="type_kind", wraps=betterproto.TYPE_NULL
    )
    ...
    # qualified message type name. For example, `google.type.PhoneNumber`.
    ...
    # with a target type of `Primitive.INT64`.
    ...
    error: protobuf.Empty = betterproto.message_field(12, group="type_kind")
    # Abstract, application defined type. An abstract type has no accessible
    # field names, and it can only be inspected via helper / member functions.
...
class Decl(betterproto.Message):
    ...
    # container, as in `cel.expr.Decl`. Declarations used as
    # [FunctionDecl.Overload][cel.expr.Decl.FunctionDecl.Overload] parameters may
    # or may not have a name depending on whether the overload is function
...
class DeclIdentDecl(betterproto.Message):
    ...
    # Documentation string for the identifier. Provide a brief description of
    # what the variable represents and whether there are any constraints on the
    # formatting or supported value range. Examples:    'request.auth.principal'
    # - string which uniquely identifies an    authenticated principal. For JSON
    # Web Tokens (JWTs), the principal    is the combination of the issuer
    # ('iss') and subject ('sub') token    fields concatenated by a forward
    # slash: iss + `/` + sub.    'min_cpus' - integer value indicates the minimum
    # number of CPUs    required for a compute cluster. The 'min_cpus' value must
    # be     greater than zero and less than 'max_cpus' or 64 whichever is less.
...
class DeclFunctionDecl(betterproto.Message):
    ...
    # Documentation string for the function that indicates the general purpose of
    # the function and its behavior. Documentation strings for the function
    # should be general purpose with specific examples provided in the overload
    # doc string. Examples:      The 'in' operator tests whether an item exists
    # in a collection.     The 'substring' function returns a substring of a
    # target string.
...
class DeclFunctionDeclOverload(betterproto.Message):
    ...
    [Expr][cel.expr.Expr] values. Functions overloads are declared in either a
    function or method call-style. For methods, the `params[0]` is the expected
    type of the target receiver. Overloads must have non-overlapping argument
    types after erasure of all parameterized type variables (similar as type
    erasure in Java).
    """
    ...
    # [Reference][cel.expr.Reference] to indicate the `overload_id` that was
    # resolved for the function `name`.
    overload_id: str = betterproto.string_field(1)
    # List of function parameter [Type][cel.expr.Type] values. Param types are
    # disjoint after generic type parameters have been replaced with the type
    # `DYN`. Since the `DYN` type is compatible with any other type, this means
    # that if `A` is a type parameter, the function types `int<A>` and `int<int>`
    # are not disjoint. Likewise, `map<string, string>` is not disjoint from
    # `map<K, V>`. When the `result_type` of a function is a generic type param,
    # the type param name also appears as the `type` of on at least one params.
    ...
    # Documentation string for the overload. Provide examples of the overload
    # behavior, preferring to use literal values as input with a comment on the
    # return value. Examples:    // Determine whether a value of type <V> exists
    # within a list<V>.    2 in [1, 2, 3] // returns true    // Determine whether
    # a key of type <K> exists within a map<K,V>.    'hello' in {'hi': 'you',
    # 'hello': 'there'}  // returns true    'help' in {'hi': 'you', 'hello':
    # 'there'}  // returns false    // Take the substring of a string starting at
    # a specific character    // offset (inclusive).    "tacocat".substring(1) //
    # returns "acocat"    "tacocat".substring(20) // error    // Take the
    # substring of a string starting at a specific character    // offset
    # (inclusive) and ending at the given offset (exclusive).
